...
export default function App() {
  ...
  const [userProfilePic, setUserProfilePic] = useState('https://images.unsplash.com/photo-1724435811349-32d27f4d5806?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwZXJzb24lMjBhdmF0YXIlMjBwcm9maWxlfGVufDF8fHx8MTc1Njc4MTIzNHww&ixlib=rb-4.1.0&q=80&w=1080');
  ...
  const sampleDuets = pins.length > 0 ? [
    {
      id: '1',
      originalPost: {
        id: pins[0].id,
  ...
  const handleCameraCapture = (photo: { image: string; caption: string; hashtags: string[] }) => {
    const newPin: Pin = {
  ...
  const handleCameraStoryUpload = (photo: { image: string; caption: string; hashtags: string[] }) => {
        ...
        return (
          <ProfileView
            stats={{
              ...
              totalLikes: pins.filter(p => p.author === 'you').reduce((sum, p) => sum + p.likes, 0),
              totalComments: pins.filter(p => p.author === 'you').reduce((sum, p) => sum + p.comments, 0),
  ...
  return (
    <div className="mobile-vh mobile-vw bg-black relative overflow-hidden">
      ...
      {/* Bottom Navigation */}
      <div className="absolute bottom-0 left-0 right-0 z-20 android-nav-bar-fix">
        <BottomNavigation
          ...
          >
            <OtherProfileView
              ...
              stats={{
                ...
                totalLikes: pins.filter(p => p.author === selectedProfileUser.id).reduce((sum, p) => sum + p.likes, 0)
              }}
